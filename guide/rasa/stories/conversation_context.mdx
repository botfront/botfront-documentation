...
meta:
  - name: description
    content: Handling conversation context with Botfront and Rasa
  - name: keywords
...
When predicting a response to **greet**, Rasa will consider the whole story, not just the latest user message.
Therefore you can re-user the same intent several times in a row and give different responses.
...
> **Since this section is about context**:
>
> The combination of intent and entities **inform** and **status** can be re-used in other contexts.
> Another story could be about asking the conditions required for an upgrade. The assistant would ask the member their status, and the user will answer with the same type of utterances as above.
> ![](../../../images/slots_entities_categorical.png)
